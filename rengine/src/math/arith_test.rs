...
mod tests {
    ...
    fn get_fixture_3() -> (Array, Array, Array) {
    ...
    fn get_fixture_unary() -> Array {
        Array::new(vec![-4.0, -1.0, 0.0, 1.0, 4.0, 9.0])
    }

    #[test]
    fn test_abs_32() {
        let result = math::arith::abs(get_fixture_unary(), Mode::Normal);
        assert_eq!(result.slice(), [4.0, 1.0, 0.0, 1.0, 4.0, 9.0]);
    }

    #[test]
    fn test_abs_32_neon() {
        let result = math::arith::abs(get_fixture_unary(), Mode::Neon);
        assert_eq!(result.slice(), [4.0, 1.0, 0.0, 1.0, 4.0, 9.0]);
    }

    #[test]
    fn test_abs_32_par_neon() {
        let result = math::arith::abs(get_fixture_unary(), Mode::ParNeon);
        assert_eq!(result.slice(), [4.0, 1.0, 0.0, 1.0, 4.0, 9.0]);
    }

    #[test]
    fn test_neg_32() {
        let result = math::arith::neg(get_fixture_unary(), Mode::Normal);
        assert_eq!(result.slice(), [4.0, 1.0, -0.0, -1.0, -4.0, -9.0]);
    }

    #[test]
    fn test_neg_32_neon() {
        let result = math::arith::neg(get_fixture_unary(), Mode::Neon);
        assert_eq!(result.slice(), [4.0, 1.0, -0.0, -1.0, -4.0, -9.0]);
    }

    #[test]
    fn test_neg_32_par_neon() {
        let result = math::arith::neg(get_fixture_unary(), Mode::ParNeon);
        assert_eq!(result.slice(), [4.0, 1.0, -0.0, -1.0, -4.0, -9.0]);
    }
    ...
    fn test_div_32_by_zero() {
        let left = Array::new(vec![1.0, 2.0, 3.0]);
        let right = Array::new(vec![0.0, 0.0, 0.0]);
